...
static void nanosleep(solo5_time_t nsecs){
    solo5_clock_nanosleep(nsecs);
}
...
int solo5_app_main(const struct solo5_start_info *si)
{
    ...
    if (solo5_sched_setscheduler(SOLO5_SCHED_FIFO, 10) != SOLO5_R_OK)
        puts("sched_setscheduler not supported\n");

    while(1) {
        long now = solo5_clock_monotonic();
        long then = now + 1000 * 1000 * 1000; /* 1s */
        nanosleep(then);
        puts("Hello");
    }
    size_t len = 0;
